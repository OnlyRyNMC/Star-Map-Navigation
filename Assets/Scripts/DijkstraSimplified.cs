...
public static class DijkstraSimplified {
	...
	private static List<OnStar> galaxyStarList;
	public static Dictionary<OnStar, float> starRoutes = new Dictionary<OnStar, float>();
 
    public static List<OnStar> FindPath(OnStar startStar, OnStar endGoalStar) {
		//Do the algorithm.
		...
		foreach (OnStar star in galaxyStarList) {
		...
		while (priorityList.Count > 0) {
			...
			foreach (StarPath nextStar in GetStarPathsFromRoutesDictionary(currentStarNode.star.DicStarRoutes, priorityList)) {
				...
				float distance = nextStar.star.DicStarRoutes[currentStarNode.star] + currentStarNode.smallestDistanceToStart;
			...
			if (priorityList.Count > 0) {
				...
				if (priorityList[0].star == endGoalStar && priorityList[0].smallestDistanceToStart != float.MaxValue) {
					...
					List<OnStar> pathToEnd = new List<OnStar>();
	...
	/// <returns></returns>
	private static List<StarPath> GetStarPathsFromRoutesDictionary(Dictionary<OnStar, float> starRoutes, List<StarPath> priorityList) {
		List<StarPath> result = new List<StarPath>();

		foreach (KeyValuePair<OnStar, float> route in starRoutes) {
            OnStar star = route.Key;
	...
	public static void SetGalaxyStarList(List<OnStar> stars) {
		galaxyStarList = new List<OnStar>(stars);
	}
    ...
    public class StarPath {
		...
		public StarPath(OnStar a_current, StarPath a_shortest) {
			star = a_current;
